...
class CorrectionAccountInvoiceLine(models.Model):
    ...
    invoice_line_gross = fields.Monetary(string='Gross', readonly=True)
    ...
    def _recompute_correction_lines(self):
        if self.quantity != self._origin.quantity or self.price_unit != self._origin.price_unit:
            if self.discount > 0:
                price_discounted = self.price_unit - (self.price_unit * (self.discount / 100))
                self.price_subtotal = price_discounted * self.quantity
            else:
                self.price_subtotal = self.price_unit * self.quantity
            self.invoice_line_tax = self.price_subtotal * self.invoice_line_tax_ids[0].amount / 100 if self.invoice_line_tax_ids else 0
            self.invoice_line_gross = self.invoice_line_tax + self.price_subtotal
        if self.invoice_line_tax_ids.ids != self._origin.invoice_line_tax_ids.ids:
            self.invoice_line_tax = self.price_subtotal * self.invoice_line_tax_ids[0].amount / 100 if self.invoice_line_tax_ids else 0
            self.invoice_line_gross = self.invoice_line_tax + self.price_subtotal
