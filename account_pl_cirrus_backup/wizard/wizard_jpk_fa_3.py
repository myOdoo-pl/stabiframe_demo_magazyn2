...
class WizardJpkFa3(models.TransientModel):
    _name = 'wizard.jpk.fa.3'
    ...
    cash_basis_pl = fields.Boolean("Cash basis", help="Select this if you are using chash basis method speciffic "
                                                      "for polish accounting.")
    ...
    def _print_report(self, data):
        ...
        jpk_file = self.env['report.jpk.fa.3'].render_xml(self)
        self.write(
            {'jpk_fa_file': jpk_file, 'jpk_fa_filename': 'JPK-FA-' + datetime.now().strftime('%Y-%m-%d') + ".xml"}
        )
        return {
