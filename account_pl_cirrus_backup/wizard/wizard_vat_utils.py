...
class WizardVatUtils(models.TransientModel):
    ...
    def get_vat(self, company_id=False, partner_id=False):
        ...
        if company_id:
            ...
            if not vat:
                if (not partner_id.property_account_position_id or partner_id.property_account_position_id.name in (
                u'Kraj', u'Wspólnota')) and partner_id.company_type == 'company':
            ...
            if not country_code.isalpha():
                raise UserError(_(
                    "Please enter correct VAT number for partner {0} - is has to begin with country-specific characters.".format(
                        partner_id.name)))
            else:
                return numeric_vat
    ...
    def get_vat_code(self, company_id=False, partner_id=False, skip_pl=False):
        ...
        if not vat:
            return False
        if partner_id and partner_id.property_account_position_id and (
                partner_id.property_account_position_id.name[:14] == u'Import/Eksport'):
    ...
    def get_address(self, company_id=False, partner_id=False):
        ...
        if not address:
            raise ValidationError(
                _('Please add address of {}'.format(company_id and company_id.name or partner_id.name)))
        ...
        for key, item in structure.items():
            ...
            if key == 'static_value':
                ...
                if item == 'loop_index':
                    ...
                    if ('numeric_vat' in item) and (('seller_data' in item) or ('purchaser_data' in item)) and (
            ...
            # don't do anything after checking value
                ...
                if item.get('no_order', False):
                    for name, value in OrderedDict(
                            values.items()).items():  # sorted method out, because VAT-7 (18)/VAT-7K (12) structures are not ordered
                ...
                for i in loop_values:
                    self.convert_to_xml(item['loop_2'], wizard, doc, data, parent=parent, loop_key=loop_key,
                                        inner_loop=i)
                loop_key = False
                ...
                if not self.convert_to_xml(item, wizard, doc, data, parent=element, loop_key=loop_key,
                                           inner_loop=inner_loop) and parent:
                    if not (element.tagName in ["SprzedazCtrl", "ZakupCtrl",
                                                "ZamowienieCtrl"]) or element.hasChildNodes():
    ...
    @api.model
    def get_vat_details(self, wizard, filter_journals=False, cash_basis=False, natural_person=False,
                        different_period_items=False):
        ...
        if cash_basis:
            ...
            payments = self.env['account.payment'].search(
                [('payment_date', '>=', wizard.date_from), ('payment_date', '<=', wizard.date_to),
                 ('payment_type', '!=', 'transfer')])
            move_payment_dict = {}
            for payment in payments.filtered(lambda
                                                     r: not r.partner_id.property_account_position_id.name or r.partner_id.property_account_position_id.name == 'Kraj'):
                ...
                partial_reconcile_ids = move_line_ids.mapped('matched_debit_ids') + move_line_ids.mapped(
                    'matched_credit_ids')
                move_lines = (partial_reconcile_ids.mapped('debit_move_id') + partial_reconcile_ids.mapped(
                    'credit_move_id')).filtered(lambda r: r not in move_line_ids)

                move = move_lines.mapped('move_id').filtered(lambda r: r.state == 'posted' and len(
                    r.line_ids.mapped('tax_line_id') + r.line_ids.mapped('tax_ids')) > 0)

                if not move:
                    continue
                elif len(move) == 1:
                    # move.ensure_one()
                    if home_currency_id in move.line_ids.mapped('currency_id') or not move.line_ids.mapped(
                            'currency_id'):
                    ...
                    if move.id not in move_payment_dict:
                        ...
                        move_payment_dict[move.id]['date'] = max(move_payment_dict[move.id]['date'],
                    ...
                    for m in move:
                        ...
                        payment_lines = move_line_ids.mapped('matched_debit_ids').filtered(
                            lambda p: p.debit_move_id == move_line).mapped('credit_move_id')
                        if not payment_lines:
                            payment_lines = move_line_ids.mapped('matched_credit_ids').filtered(
                                lambda p: p.credit_move_id == move_line).mapped('debit_move_id')
            ...
            other_moves = AccountMove.search([
                ...
                ('state', '=', 'posted')
                                  ...
                                  ((r.line_ids.mapped('tax_line_id') or r.line_ids.mapped('tax_ids')) and (
                                              not r.line_ids.mapped('invoice_id') or \
                                              (any(move_type in ['in_refund', 'out_refund'] for move_type in
                                                   r.line_ids.mapped('invoice_id.move_type'))))))
        ...
        for move in sale_move_ids:
            ...
            if different_dates_lines:
                for line in different_dates_lines:
                    ...
                    if not base_tax or not net_lines:
                        net_lines = [move_line.id for move_line in move.line_ids if
            ...
            partner_data = {
                ...
                # if no numeric vat the vat = false because country out of EU
            }
            ...
            if inv:
                ...
                negative_deposit_line = deposit_line if deposit_line and sum(
                    deposit_line.mapped('price_subtotal')) < 0 else False
                ...
                if inv.reversed_entry_id:
                    ...
                    refunded_period = (
                        refund_inv_id.date_issue if refund_inv_id.date_issue else refund_inv_id.invoice_date).strftime(
                        '%m.%Y')
                ...
                if negative_deposit_line:
                    dp = self.prepare_line_value(inv, company_id, abs(sum(negative_deposit_line.mapped('price_total'))),
                                                 compute=True)
                ...
                for line in inv.invoice_line_ids.filtered(
                        lambda r: (r not in (negative_deposit_line or [])) and (r.display_type == False)):
                    fa_lp += 1
                    sum_invoice_line += line.price_subtotal

                    # unit_gross = line.price_unit
                    price_unit = self.prepare_line_value(inv, company_id,
                                                         line.price_unit - line.price_unit * down_payment_discount,
                                                         True, True)
                    unit_gross = self.prepare_line_value(inv, company_id,
                                                         line.price_unit - line.price_unit * down_payment_discount,
                                                         True, True)

                    if line.tax_ids:
                        unit_taxes = line.tax_ids.compute_all(line.price_unit, inv.currency_id, 1,
                                                              product=line.product_id, partner=partner_id)
                        # unit_gross = unit_taxes['total_included']
                        unit_taxes_value = unit_taxes['total_included']
                        unit_gross = self.prepare_line_value(inv, company_id,
                                                             unit_taxes_value - unit_taxes_value * down_payment_discount,
                                                             True, True)

                    discount = self.prepare_line_value(inv, company_id, (line.price_unit * line.quantity) * (
                                (line.discount / 100) + down_payment_discount), True, True)
                    subtotal = self.prepare_line_value(inv, company_id,
                                                       line.price_subtotal - line.price_subtotal * down_payment_discount,
                                                       round=True)
                    gross = self.prepare_line_value(inv, company_id,
                                                    line.price_total - line.price_total * down_payment_discount, True,
                                                    True)
                ...
                seller_data = company_data.copy() if inv.move_type in ['out_invoice',
                                                                       'out_refund'] else partner_data.copy()
                purchaser_data = partner_data.copy() if inv.move_type in ['out_invoice',
                                                                          'out_refund'] else company_data.copy()

                if (
                        not partner_id.property_account_position_id or partner_id.property_account_position_id.name == 'Kraj') and inv.move_type in [
                    'out_invoice', 'out_refund'] and cash_basis:
            ...
            for move_line in move.line_ids:
                ...
                if move_line.id in different_month or move_line.id in this_month:
                    ...
                    if different_period_items:
                        ...
                        if move_line.date_manual:
                            if not (
                                    move_line.date_manual >= wizard.date_from and move_line.date_manual <= wizard.date_to):
                                continue
                ...
                if wizard._name == 'wizard.jpk.vat.2020' and (
                        wizard.date_from.month != wizard.date_to.month and wizard.period != 1) and wizard.no_declaration_data == False:
                    if move_line.date_manual:
                ...
                if move_line.tax_basic_custom > 0.0:
                    ...
                    if netto_name in TAXES_UE.keys():
                        ...
                        if partner_data['vat'] not in ue_dict[ue_name]:
                            ue_dict[ue_name][partner_data['vat']] = {'country_code': partner_data['country_code'],
                                                                     'vat': partner_data['numeric_vat'], 'amount': 0.0}
            ...
            if len(expected_taxes) != len(taxes_check) or len(set(expected_taxes) - set(taxes_check)) > 0:
                raise UserError(
                    _("Error ocured while JPK taxes validation. Please correct taxes in Journal Entry %s") % move.name)
        ...
        purchas_move_ids = moves.filtered(
        ...
        for move in purchas_move_ids:
            ...
            for move_line in move.line_ids:
                ...
                if netto_name in purchase_taxes.keys() and purchase_taxes[netto_name][0] in move_taxes.keys() and len(
                        purchase_taxes[netto_name]) == 2 and purchase_taxes[netto_name][1] not in move_taxes.keys():
        ...
        if wizard._name == 'wizard.jpk.vat.2020':
            vat_data.update({'P_38': (vat_data['P_16'] + vat_data['P_18'] + vat_data['P_20'] + vat_data['P_24'] +
                                      vat_data['P_26'] + vat_data['P_28']
                                      + vat_data['P_30'] + vat_data['P_32'] + vat_data['P_33'] + vat_data['P_34']) - (
                                                 vat_data['P_35'] - vat_data['P_36'])})
            vat_data.update({'P_48': (
                        vat_data['P_39'] + vat_data['P_41'] + vat_data['P_43'] + vat_data['P_44'] + vat_data['P_45'] +
            ...
            if (vat_data['P_48']) - vat_data['P_38'] >= 0:
                ...
                vat_data.update({
                                    'P_54': wizard.p_54_vdek})  # jeżeli jest zwrot to te pole musibyć w xml plus zaznaczone któreś z klejnych pól 55,56,57,58
            else:
    ...
    def get_account_details(self, wizard, acc_details):
        ...
        journal_details = self.env['account.move'].search(
            [('date', '>=', wizard.date_from), ('date', '<=', wizard.date_to)], order='date')
        for line in journal_details:
            ...
            journal_dict[count_j] = {
                ...
                'operation_descr': line.ref if line.ref else line.partner_id.display_name or '/',
                # [move_line.name for move_line in line.line_ids][0] or
        ...
        acc_move_line_details = self.env['account.move.line'].search(
            [('date', '>=', wizard.date_from), ('date', '<=', wizard.date_to)], order='date')
    ...
    def get_bank_details(self, wizard):
        ...
        balance_details = self.env['account.bank.statement'].search(
            [('journal_id', '=', wizard.journal_id.id), ('date', '>=', wizard.date_from),
        ...
        if 'balance_start' in balance:
            ...
            raise UserError(_(
                "Please check if the details e.g., dates you provided are correct or at least one bank statement is present for the given date!"))

        balance_lines_details = self.env['account.bank.statement.line'].search(
            [('journal_id', '=', wizard.journal_id.id), ('date', '>=', wizard.date_from),
             ('date', '<=', wizard.date_to)], order='date')
