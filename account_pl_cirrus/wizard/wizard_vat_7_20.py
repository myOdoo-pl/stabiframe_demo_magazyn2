...
class AccountReportVat7_20(models.TransientModel):
    _name = "account.report.vat.7.20"
    ...
    def _print_vat7(self, data):
        if self.env.context.get('xml_version'):
            vat7_file = self.env[
                'report.account_pl_cirrus.report_vat_7_20'].render_xml(data)
            ...
            return self.env.ref('account_pl_cirrus.account_vat_7_20').report_action(self, data=data)

    def _print_vat7k(self, data):
        if self.env.context.get('xml_version'):
            vat7k_file = self.env['report.account_pl_cirrus.report_vat_7k_14'].render_xml(data)
            ...
            pprint(self.env.context)
            return self.env.ref('account_pl_cirrus.account_vat_7k_14').report_action(self, data=data)
