...
class AccountReportVat7_18(models.TransientModel):
    _name = "account.report.vat.7.18"
    ...
    def _print_vat7(self, data):
        if self.env.context.get('xml_version'):
            vat7_file = self.env[
                'report.account_pl_cirrus.report_vat_7_18'].render_xml(data)
            ...
            return self.env.ref('account_pl_cirrus.account_vat_7_18').report_action(self, data=data)

    def _print_vat7k(self, data):
        if self.env.context.get('xml_version'):
            vat7k_file = self.env['report.account_pl_cirrus.report_vat_7k_12'].render_xml(data)
            ...
            pprint(self.env.context)
            return self.env.ref('account_pl_cirrus.account_vat_7k_12').report_action(self, data=data)
